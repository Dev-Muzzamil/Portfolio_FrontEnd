...
import { ValidatedInput } from '../common/FormValidation';
import { LoadingButton, ActionLoading } from '../common/LoadingStates';
import toast from 'react-hot-toast';

/**
 * CertificatesManagementUnified
 * Comprehensive certificate management component for admin dashboard
 * 
 * Features:
 * - Create, read, update, delete certificates
 * - Automatic data extraction from PDF/image uploads
 * - File management (upload, delete, set primary)
 * - Report management
 * - Project linking
...
const CertificatesManagementUnified = () => {
  ...
  const { endUploadSession } = useAuth();
  const { register, handleSubmit, formState: { errors }, reset, control, trigger } = useForm();
  ...
  const [, setPreviewFile] = useState(null);
  ...
  const createCertificateWithFiles = async (certificateData, files) => {
    try {
      ...
      formData.append('action', 'create-with-files');
      ...
      const response = await axios.post('/api/v1/admin/content/certificates/actions', formData, {
        headers: {
                  ...
                  render={({ field }) => (
                    <InstitutionSelector
                      ...
                      onChange={(value) => {
                        ...
                        trigger('completedAtInstitution');
                      }}
