...
const CertificateReportsManager = ({ certificateId, reports = [], onUpdate }) => {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      formData.append('title', 'Certificate Report');
      formData.append('description', 'Certificate documentation and reports');

      const response = await fetch(`/api/certificates/${certificateId}/reports/upload`, {
        method: 'POST',
        headers: {
  ...
  const handleAddLink = async () => {
    ...
    try {
      const response = await fetch(`/api/certificates/${certificateId}/reports/link`, {
        method: 'POST',
        headers: {
  ...
  const handleDeleteReport = async (reportId) => {
    ...
    try {
      const response = await fetch(`/api/certificates/${certificateId}/reports/${reportId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const handleToggleVisibility = async (reportId, visible) => {
    try {
      const response = await fetch(`/api/certificates/${certificateId}/reports/${reportId}/visibility`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
...
export default CertificateReportsManager;
